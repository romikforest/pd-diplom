...
# class CustomDefaultRouterMixin(object):
#     """
#     Миксин для DefaultRouter
#     Роутер генерирует пути для конечного слеша и без него
#     Он также старается добавить простые пути в api root view, не только list
...
#     def get_urls(self):
#         urls = super(CustomDefaultRouterMixin, self).get_urls()
#         for i in range(len(urls)):
#             regex = str(urls[i].pattern)
#             if regex[-2:] == '/$':
#                 regex = regex[:-2] + '/?$'
...
# class CustomSimpleRouterMixin(object):
#     """
#     Миксин для SimpleRouter
#     Роутер генерирует пути для конечного слеша и без него
#     """
...
#     def get_urls(self):
#         urls = super(CustomSimpleRouterMixin, self).get_urls()
#         for i in range(len(urls)):
#             regex = str(urls[i].pattern)
#             if regex[-2:] == '/$':
#                 regex = regex[:-2] + '/?$'
...
# class CustomDefaultRouter(CustomDefaultRouterMixin, routers.DefaultRouter):
#     """
#     Роутер генерирует пути для конечного слеша и без него
#     Он также старается добавить простые пути в api root view, не только list
#     С помощью свойств root_view_pre_items и root_view_post_items
...
# class CustomSimpleRouter(CustomSimpleRouterMixin, routers.SimpleRouter):
#     """
#     Роутер генерирует пути для конечного слеша и без него
#     """
#     pass
...
# class CustomNestedDefaultRouter(CustomDefaultRouterMixin, routers.NestedDefaultRouter):
#     """
#     Вложенный роутер генерирует пути для конечного слеша и без него
#     Он также старается добавить простые пути в api root view, не только list
#     С помощью свойств root_view_pre_items и root_view_post_items
#     можно добавить дополнительные поля в api root view
#     соответсвенно до и после зарегистрированных роутов
...
# class CustomNestedSimpleRouter(CustomSimpleRouterMixin, routers.NestedSimpleRouter):
#     """
#     Вложенный роутер генерирует пути для конечного слеша и без него
#     """
#     pass
